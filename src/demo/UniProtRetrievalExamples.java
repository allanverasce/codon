...
public final class UniProtRetrievalExamples {
    //private final static Logger logger = LoggerFactory.getLogger(UniProtRetrievalExamples.class);
    ...
    public static void driveExamplesMine () {
        ServiceFactory serviceFactoryInstance = Client.getServiceFactoryInstance();
        UniProtService uniProtService = serviceFactoryInstance.getUniProtQueryService();
        try {
            // start the service
    ...
    public static void driveExamples() {
        ...
        try {
            ...
            // the accession we're interested in
            String accession = "Q3Z5Y1";

            UniProtEntry entry = uniProtService.getEntry(accession);
            if (entry == null) {
                res = "Entry " + accession + " could not be retrieved";
            } else {
                res = entry.getSequence ().getValue ();
            }
